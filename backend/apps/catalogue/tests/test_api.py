...
The catalogue is public, so the important checks are that it is reachable
WITHOUT a token and that inactive rows never leak.
"""
...
def test_services_of_inactive_category_are_hidden(client, catalogue):
    """A deactivated category must take its services out of the listing."""
    electrical, _ = catalogue
    electrical.is_active = False
    electrical.save(update_fields=["is_active"])
...
def test_catalogue_is_not_writable_through_the_api(client, catalogue):
    """Admin manages the catalogue; there is no public write path."""
...
def test_categories_returned_in_display_order(client, catalogue):
    """
    Regression: annotate(Count(...)) adds a GROUP BY that silently discards
    Meta.ordering, so the landing page rendered its categories in arbitrary,
    deploy-dependent order. The selector re-applies ordering explicitly.
    """
