...
DOC = [
    ...
    ("code", [
        ...
        "-- this. Extensions must be created by the superuser first.",
        "GRANT ALL ON SCHEMA public TO sheba;",
        "ALTER SCHEMA public OWNER TO sheba;",
        "",
        "-- Verify before moving on: should print a distance in miles.",
        "SELECT point(90.4125,23.8103) <@> point(90.3654,23.7461);",
        "",
        "# 2. Backend",
        "cd backend",
        "python -m venv venv",
    ...
    ("p", "Target: a single Ubuntu 22.04 LTS VPS with 2 GB RAM. Services are "
    ...
    ("table", {
        ...
        "rows": [
            ["Web server", "Nginx", "Terminates TLS, serves the built React "
             "bundle and <font face='Courier'>/media</font>, proxies "
             "<font face='Courier'>/api</font> to Gunicorn."],
            ...
            ["Scheduler", "cron", "One crontab entry per management command, "
             "mirroring the schedule in section 8.4."],
            ["Backups", "pg_dump + cron", "Nightly dump to off-server storage, "
             "30-day retention, restore tested before launch."],
        ]
    }),
    ("code", [
        "# Production crontab - the server-side equivalent of 11.3",
        "*/15 * * * *  cd /srv/sheba && venv/bin/python manage.py expire_requests",
        "0    * * * *  cd /srv/sheba && venv/bin/python manage.py auto_confirm",
        "*/30 * * * *  cd /srv/sheba && venv/bin/python manage.py send_reminders",
        "*/10 * * * *  cd /srv/sheba && venv/bin/python manage.py purge_otps",
        "0    3 * * *  cd /srv/sheba && venv/bin/python manage.py recompute_all_trust",
        "30   3 * * *  cd /srv/sheba && venv/bin/python manage.py purge_documents",
        "0    2 * * *  /srv/sheba/scripts/backup.sh",
    ]),
